...
module.exports.fetchUser = (req, res) => {
    if (req.params.id !== undefined) {
        database.models['user'].findOne({ _id: req.params.id }).populate('group').exec((err, data) => {
            if (err) return res.status(500).send({ error: 'Error retriving user data.' })
            return res.status(200).send(data)
        })
    }
    else {
        database.models['user'].find({}).populate('group').exec((err, data) => {
            if (err) return res.status(500).send({ error: 'Error retriving user data.' })
            let sortedUsers = data.sort((a, b) => a.username.localeCompare(b.username))
            return res.status(200).send(data)
        })
    }
}
...
module.exports.updateUser = (req, res) => {
    ...
    if (req.body.password === undefined) req.body.password = bcrypt.hashSync('password', bcrypt.genSaltSync(10))
    ...
    database.models['user'].findOne({ _id: res.locals.id }).exec((err, data) => {
        bcrypt.compare(req.body.oldPassword, data.get('password', null, { getters: false }), (error, match) => {
            if (match && req.body.newPassword === req.body.confirmPassword) {
                data.password = bcrypt.hashSync(req.body.newPassword, bcrypt.genSaltSync(10));
                data.save();
                return res.status(200).send({});
            }
