            ...
            for (const game of submission.games) {
                for (const category of game.categories) {
                    if (selections[category.id] !== undefined && selections[category.id].status === 'VALIDATED') {
                        ...
                        if (runner === null) runnerArray.push(await createRunner(submission.user));
                        else runnerArray.push(runner._id);
                        for (const gameRunner of category.opponentDtos) {
                            ...
                            else runnerArray.push(opponentRunner._id);
                        }
                        let speedrun = {
                        ...
                        await database.models['run'].create(speedrun);
                    }
                }
            }
        }
...
async function createRunner(runner) {
    let runnerProfile = {
        ...
        email: '',
        pronouns: (runner.pronouns !== undefined && runner.pronouns !== null) ? runner.pronouns : '',
    };
