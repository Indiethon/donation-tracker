...
module.exports.fetchSpeedrun = (req, res) => {
    if (req.params.id !== undefined) {
        ...
        database.models['run'].find({ eventId: res.locals.event }).populate('runners').exec((err, data) => {
            if (err) return res.status(500).send({ error: 'Error retriving speedrun data.' })
...
module.exports.importSpeedruns = async (req, res) => {
    ...
    if (req.params.event !== undefined) {
        let schedule;
        let submissions = [];
        const submissionInfo = await fetch(`https://oengus.io/api/v1/marathons/${req.body.short}/submissions`).then(subRes => subRes.json()).catch(() => {
            return res.status(404).send({ error: 'Marathon short is invalid.', errorCodes: [{ item: 'short', code: 'Marathon short is invalid.' }] });
        });
        submissions = submissionInfo.content;
        const getSubmissions = new Promise(async (resolve, reject) => {
            for (let i = 2; i <= submissionInfo.totalPages; i++) {
                let data = await fetch(`https://oengus.io/api/v1/marathons/${req.body.short}/submissions?page=${i}`).then(subRes => subRes.json())
                submissions.push.apply(submissions, data.content)
                if (i === submissionInfo.totalPages) return resolve();
            }
        })

        await Promise.all([getSubmissions])
        const selections = await fetch(`https://oengus.io/api/v1/marathons/${req.body.short}/selections`).then(selRes => selRes.json());
        if (req.body.schedule) {
            schedule = await fetch(`https://oengus.io/api/v1/marathons/${req.body.short}/schedule`).then(schData => schData.json());
        ...
        if (!submissions.type && submissions.type === "NotFoundException") return res.status(404).send({ error: 'Marathon short is invalid.', errorCodes: [{ item: 'short', code: 'Marathon short is invalid.' }] });
        for (const submission of submissions) {
            for (const game of submission.games) {
                for (const category of game.categories) {
                    if (selections[category.id] !== undefined && (selections[category.id].status === 'VALIDATED' || selections[category.id].status === 'BONUS')) {
                        let runnerArray = [];
