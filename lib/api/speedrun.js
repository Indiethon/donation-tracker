...
module.exports.fetchSpeedrun = (req, res) => {
    ...
    else if (req.query.event !== undefined) {
        database.models['run'].find({ eventId: req.query.event }).populate('runners').exec((err, data) => {
            if (err) return res.status(500).send({ error: 'Error retriving speedrun data.' })
...
module.exports.importSpeedruns = async (req, res) => {
    ...
    if (req.param.event !== undefined) {
        let schedule;
        const submissions = await fetch(`https://oengus.io/api/v1/marathons/${req.body.short}/submissions`).then(subRes => subRes.json());
