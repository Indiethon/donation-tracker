...
module.exports = (tracker, database) => {
    ...
    async function verify(req, res) {
        ...
        if (req.body.model === null || user.superuser || (permission !== undefined && permissionArray.indexOf(permission.level) >= permissionArray.indexOf(req.body.level))) return res.status(200).send({ admin: user.admin, volunteer: user.volunteer })       
