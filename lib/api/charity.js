...
module.exports.fetchCharity = (req, res) => {
    if (req.params.id !== undefined) {
        database.models['charity'].findOne({ _id: req.params.id }).exec((err, data) => {
            if (err) return res.status(500).send({ error: 'Error retriving charity data.' })
            data.getStats((cb) => {
        ...
        database.models['charity'].find({}).exec((err, data) => {
            if (err) return res.status(500).send({ error: 'Error retriving charity data.' })
            return res.status(200).send(data)
        })
    }
...
module.exports.deleteCharity = (req, res) => {
    if (req.params.id !== undefined) {
        database.models['charity'].deleteOne({ _id: req.params.id }, (err) => {
...
module.exports.updateCharity = (req, res) => {
    ...
    database.models['charity'].create(req.body, (err, data) => {
...
//     else if (data.name.length > 60) errors.push({ item: 'name', code: 'Charity name is too long.' })
//     else {
