...
function validateEvent(data, update, callback) {
    ...
    if (data.short === undefined || data.short.length <= 0) errors.push({ item: 'short', code: 'Short is required.' })
        ...
        shortUnique = new Promise((resolve, reject) => {
            database.find('event', { short: data.short }, (err, shortData) => {
                ...
                else if (shortData !== undefined && update && data.short !== shortData.short) errors.push({ item: 'name', code: 'Short is already in use.' })
                else if (shortData !== undefined) errors.push({ item: 'short', code: 'Short is already in use.' })
                resolve();
            })
        })
    }
    ...
    if (data.endTime === undefined || data.endTime.length <= 0) errors.push({ item: 'endTime', code: 'End time is required.' })
        ...
        if (data.startTime === undefined || typeof data.startTime !== 'date' || dateString.getTime() <= data.startTime.getTime()) errors.push({ item: 'endTime', code: 'End time must be after the start time.' })
