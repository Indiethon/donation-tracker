...
module.exports.claimPrize = async (req, res) => {
    let deCipher = crypto.createDecipheriv('aes-256-cbc', tracker.config.prizes.prizeLinkKey, req.params.encryptedData.substring(0, 16));
    let decryptedData;
    try {
        decryptedData = deCipher.update(req.params.encryptedData.substring(16), "hex", 'utf-8');
        decryptedData += deCipher.final('utf8')
    } catch { return res.sendFile(path.join(__rootdir, 'pages/prizeRedeem/error.html')) }

    let idString = decryptedData.split('/');

    database.models['prizeRedemption'].findOne({ $and: [{ donorId: idString[0] }, { prizeId: idString[1] }] }).exec(async (err, data) => {
        if (err) return res.sendFile(path.join(__rootdir, 'pages/prizeRedeem/error.html'))
        ...
        if (Date.now() > expiry) {
            ...
            data.save();
            return res.sendFile(path.join(__rootdir, 'pages/prizeRedeem/error.html'))
        }

        let prize = await database.models['prize'].findOne({ _id: idString[1] });
        let prizeArray = prize.winners.map(x => x.toString());
        if (!prizeArray.includes(idString[0]) || data.status !== 'pending') return res.sendFile(path.join(__rootdir, 'pages/prizeRedeem/error.html'))
        data.status = 'claimed';
        data.responseTimestamp = Date.now();
        data.save();
        if (data.claimEmailTemplate) email.sendRedemptionEmail(data);
        return res.sendFile(path.join(__rootdir, 'pages/prizeRedeem/success.html'))
    })
}
...
module.exports.forfeitPrize = async (req, res) => {
    ...
    database.models['prizeRedemption'].findOne({ $and: [{ donorId: idString[0] }, { prizeId: idString[1] }] }).exec(async (err, data) => {
        ...
        data.responseTimestamp = Date.now();
        data.save();
        return res.sendFile(path.join(__rootdir, 'pages/prizeRedeem/forfeit.html'))
    })
}
