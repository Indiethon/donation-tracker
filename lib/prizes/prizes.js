...
module.exports.claimPrize = async (req, res) => {
    let deCipher;
    try { deCipher = crypto.createDecipheriv('aes-256-cbc', tracker.config.prizes.prizeLinkKey, req.query.data.substring(0, 16)); } catch(e) { console.error(e); return res.redirect('/prize/claim/error') }
    let decryptedData;
    try {
        decryptedData = deCipher.update(req.query.data.substring(16), "hex", 'utf-8');
        decryptedData += deCipher.final('utf8')
    } catch(e) { console.error(e); return res.redirect('/prize/claim/error') }
...
module.exports.forfeitPrize = async (req, res) => {
    let deCipher;
    try { deCipher = crypto.createDecipheriv('aes-256-cbc', tracker.config.prizes.prizeLinkKey, req.query.data.substring(0, 16)); } catch(e) { console.error(e); return res.redirect('/prize/forfeit/error') }
    let decryptedData;
    try {
        decryptedData = deCipher.update(req.query.data.substring(16), "hex", 'utf-8');
        decryptedData += deCipher.final('utf8')
